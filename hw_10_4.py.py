...
# 7. Move the cursor to the end of first line of the string
def cmd_7(head, tail, cursor_node, cursor): 
    if cursor_node[PREV_PTR] is not None:
        cursor_node = cursor_node[PREV_PTR]
        new_line = cursor_node[0]
        if len(new_line) < cursor:
            cursor = len(new_line)
    if cursor < len(x_list):
        cursor = len(x_list)
...
head, tail, cursor_node, cursor = cmd_7(head, tail, cursor_node, cursor)
print_file_double_list(head, tail, cursor_node, cursor)
